...
resource "google_pubsub_subscription" "inspectbigquerytable_subscription" {
  name  = "inspectbigquerytable-subscription"
  topic = google_pubsub_topic.inspectbigquerytable.name
  push_config {
    push_endpoint = google_cloud_run_service.inspectbigquerytable_service.status[0].url
    ...
    oidc_token {
      service_account_email = google_service_account.inspectbigquerytable_cloud_run_service_account.email
    }
    # https://cloud.google.com/pubsub/docs/push#authentication_and_authorization
  }
  dead_letter_policy {
    dead_letter_topic     = google_pubsub_topic.inspectbigquerytable.id
